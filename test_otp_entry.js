...
async function testOtpEntry() {
  ...
  try {
    ...
    const inputLocators = page.locator('input');
    const count = await inputLocators.count();
    
    for (let i = 0; i < Math.min(count, 6); i++) {
      await inputLocators.nth(i).fill(otpDigits[i]);
    }
    
    await page.screenshot({ path: 'test_otp_entered.png' });
    
    console.log('Clicking Verify Code...');
    await page.locator('button:has-text("Verify Code")').click();
    await page.waitForTimeout(4000);
    
    await page.screenshot({ path: 'test_form2_agency.png' });
